...
// CODE HERE
function multiply(firstNum, secNum, callback){
  callback(firstNum * secNum)
}
...
function printNames(uniqArr){
  console.log('The new names array with all the diplicate items removed is' +
  uniqArr)
}

function uniq(array, callback){
  for(let i = 0; i < array.length - 1; i++){
    for(let t = 1; t < array.lenght -1; t++){
      if (array[i] === array[t]){
        array.splice(t,1)
      }
    }
...
function callBackEach(item, index){
