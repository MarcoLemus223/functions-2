...
// CODE HERE
function flilterToEven(element){
  if(element % 2 === 0){
    return true
  } else {
    return false
...
const evenNumbers = mixedNumbers.filter(flilterToEven)
...
// CODE HERE
function calcWithTax(oldPrice){
  return oldPrice * 1.07
}

const postTaxPrices = prices.map(calcWithTax)
...
// CODE HERE
function sumNum(total, next){
  return total + next
}
const totalPopulation = populations.reduce(sumNum)
...
// CODE HERE
function strongestMonsters(monster){
  if(monster['CP'] > 200){
    return true
  }else{
    return false
...
const myStrongest = monstersInYourPocket.filter(strongestMonsters)
...
function getTotal(item){
  this.push['price'] * (1 + item['tax'])
}

newArray = []
orders.forEach(getTotal, newArray)
...
function checkForBob(total, current){
...
let bobSum = purchases.reduce(checkForBob, 0)
console.log(bobSum)